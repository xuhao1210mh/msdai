...
<html>
  ...
  <body>
    ...
    <div class="x-body">
      ...
      <xblock>
        <button class="layui-btn" onclick="x_admin_show('添加用户','./adminAdd')"><i class="layui-icon"></i>添加</button>
      </xblock>
        ...
        <tbody>
          <?php if(is_array($result)): $i = 0; $__LIST__ = $result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
              ...
              <td class="td-manage">
                ...
                <a title="编辑"  onclick="x_admin_show('编辑','adminEdit/?uid=<?php echo ($val["uid"]); ?>')" href="javascript:;">
                  <i class="layui-icon">&#xe642;</i>
                </a>
    ...
    <script>
      ...
      function member_del(obj,id){
          layer.confirm('确认要删除吗？',function(index){
              //发异步删除数据
              var uid = id;//$(obj).parent().parent().parent().find('td').eq(1).text();
              //发异步删除数据
              $.ajax({
                url: "<?php echo U('/admin/admin/adminList');?>",
                type: "post",
                dataType: "json",
                data: {
                    uid: uid,
